...
import { chargeWaitDetail, dialsInForce, inboundRows, latestDay, planSeries, planSummary, type DialPromotion, type KpiFive, type SitePowerPlan } from "../plan";
...
// otto-q-core 0501 (G233): the wait for a charger, beside KPI 5. The numbers are the query behind 0501 on validation
// run 394e1e83, whose KPI 5 read 0.7 minutes while 42 cars were still waiting for a charger when it stopped. The same
// four cases and the same words as the twin cockpit's KPI tab (ottoyarddepot-sim src/lib/chargeWait.ts).
